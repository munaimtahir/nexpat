...
# Anonymous historical patients don't provide a phone number. Using ``None`` avoids
# storing a bogus sentinel value that could be mistaken for a real number in future
# searches or reports.
...
def _normalise_gender(raw_value):
    """Map historical gender values to the canonical choices used by Visit/Patient."""
...
def forwards_func(apps, schema_editor):
    ...
    # 2. Migrate existing visits in small atomic batches so SQLite keeps type coercion
    # predictable. We only touch visits missing either patient or queue references.
    visits_base_qs = (
        ...
        with transaction.atomic(using=db_alias):
            ...
            for visit in batch_visits:
                ...
                if visit.patient_id is None:
                    name = (getattr(visit, "patient_name", "") or "").strip() or ANONYMOUS_NAME_FALLBACK
